...
table(accepts$bad)
accepts$bad-1 #0s become -1 and 1s become 0s. take absolute value. 

accepts$good <- abs(accepts$bad - 1)
table(accepts$good)
table(accepts$app_id)
...
# When we smbinn then we find that all the categorical fctors are out cuz low IV value (bankruptcy used_ind and prupose are out)
smbinning.sumiv.plot(iv_summary)
iv_summary 

# Only Use Continuous Variables >= 0.1 IV #
...
check_ress <- smbinning(df = train, y = "good", x = "loan_amt")
check_ress <- smbinning(df = train, y = "good", x = "ltv")
check_ress <- smbinning(df = train, y = "good", x = "down_pyt")
check_ress <- smbinning(df = train, y = "good", x = "bankruptcy")
check_ress

#For loop just fo rnumeric variables
for(i in 1:length(num_names)){
  ...
  if(check_res[1] == "No significant splits") {
    next
  }
  if(check_res$iv < 0.1) { #in this clause down payment gets excluded from result_all_sig list as its IV is 0.085
    next
  }
  else {
...
result_all_sig # this is a list that u will loop over, this list contains only sig splits variables

# Generating bins for only significant Variables that got binned  and WOE Values. 
# Creating new columns showing bin splits row by row plus woe values
...
#Loop velow generates new columns with WOE values for every obs every column
...
# Build Initial Logistic Regression on WOEs #rare events problem, created weights to oversample and balance 
...
# Variable Selected Logistic Regression  p values. take out the first 2 commented at top
...
smbinning.metrics(dataset = train, prediction = "pred", actualclass = "bad", report = 1) #report=1 gives all metrics
...
#TEST DATA
#Now repeat everything on Testing Data #
for(i in 1:length(result_all_sig)) {
